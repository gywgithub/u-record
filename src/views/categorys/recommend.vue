<template>
    ...
    <van-search
            ...
            <template #action>
                <div @click="toCategorys" class="font20 categoryList"><van-icon name="wap-nav" /></div>
            </template>
            <template #left>
                <van-popover
                        ...
                        @select="onSelect">
                    <template #reference>
                        <div><van-icon name="add-o setFontSize" /></div>
                    </template>
                </van-popover>
            </template>
    </van-search>
    <van-pull-refresh v-model="state.refreshing" @refresh="onRefresh">
        <van-list
                ...
                @load="onLoad">
            <van-cell-group>
                <van-cell v-for="item in state.list" :key="item" :title="item.title" @click="toDetailPage(item)">
                    <div class="thingWarp">
...
<script>
    ...
    export default defineComponent({
        setup() {
            ...
            const actions = ref([{ id:1,text: '写文章' , icon: 'records'}, { id:2,text: '写随笔', icon: 'balance-list-o' }]);
